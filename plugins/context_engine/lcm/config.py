...
def _hermes_lcm_setting(key: str, default):
    """Read a value from the ``lcm:`` section of ~/.hermes/config.yaml.

    Lets the model-free / deterministic-only switch live in config.yaml so it
    persists without exporting an env var. ``LCM_*`` env vars still override.
    """
    ...
    try:
        ...
        if yaml is not None:
            ...
            lcm = cfg.get("lcm") or {}
            if isinstance(lcm, dict) and key in lcm:
                return lcm[key]
        return default
    except Exception:
        return default
...
def _hermes_lcm_bool(key: str, default: bool) -> bool:
    value = _hermes_lcm_setting(key, None)
    if value is None:
        return default
    if isinstance(value, bool):
        return value
...
class LCMConfig:
    ...
    # When True, LCM never calls an LLM. Compaction skips L1/L2 LLM summaries
    # and uses deterministic L3 truncation only. The lossless DAG and the
    # retrieval tools (lcm_grep/lcm_expand/lcm_describe) keep working — only the
    # model round-trips are removed. Defaults to True in this fork (model-free).
    ...
    def from_env(cls) -> "LCMConfig":
        ...
        c.deterministic_only = _parse_bool_env(
            "LCM_DETERMINISTIC_ONLY",
            _hermes_lcm_bool("deterministic_only", c.deterministic_only),
        )
