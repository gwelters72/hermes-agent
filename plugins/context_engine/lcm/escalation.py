...
def summarize_with_escalation(
    ...
    When ``deterministic_only`` is set, the LLM levels (L1/L2) are skipped
    entirely and compaction goes straight to deterministic L3 truncation —
    no model is ever called.
    """
    if deterministic_only:
        l3_result = _deterministic_truncate(text, l3_truncate_tokens)
        logger.debug(
            "LCM deterministic-only mode: L3 truncation (%d tokens), no LLM call",
            count_tokens(l3_result),
        )
        return l3_result, 3
